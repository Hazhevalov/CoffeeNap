...
public partial class SettingsPage : ContentPage
{
    ...
    private void OnViewModelPropertyChanged(object? sender, PropertyChangedEventArgs eventArgs)
    {
        ...
        {
            ScheduleLanguageMenuTransition(_viewModel.IsLanguagePanelVisible);
        }
        else if (eventArgs.PropertyName == nameof(SettingsPageViewModel.IsDeleteConfirmationVisible))
        {
            ScheduleDeleteWarningTransition(_viewModel.IsDeleteConfirmationVisible);
        }
    }
        ...
        try
        {
            ...
            do
            {
                ...
                if (animatedTarget)
                {
                    ...
                    LanguageMenu.TranslationY = -8;
                    await Task.WhenAll(
                        LanguageMenu.FadeToAsync(1, LanguageMenuDuration, Easing.CubicOut),
                        LanguageMenu.TranslateToAsync(0, 0, LanguageMenuDuration, Easing.CubicOut));
                }
                else if (LanguageMenu.IsVisible)
                {
                    LanguageMenu.InputTransparent = true;
                    await Task.WhenAll(
                        LanguageMenu.FadeToAsync(0, LanguageMenuDuration, Easing.CubicIn),
                        LanguageMenu.TranslateToAsync(0, -8, LanguageMenuDuration, Easing.CubicIn));
        ...
        try
        {
            ...
            do
            {
                ...
                if (animatedTarget)
                {
                    ...
                    DeleteWarning.Scale = 0.96;
                    await Task.WhenAll(
                        DeleteWarning.FadeToAsync(1, DeleteWarningDuration, Easing.CubicOut),
                        DeleteWarning.ScaleToAsync(1, DeleteWarningDuration, Easing.CubicOut));
                }
                else if (DeleteWarning.IsVisible)
                {
                    DeleteWarning.InputTransparent = true;
                    await Task.WhenAll(
                        DeleteWarning.FadeToAsync(0, DeleteWarningDuration, Easing.CubicIn),
                        DeleteWarning.ScaleToAsync(0.96, DeleteWarningDuration, Easing.CubicIn));
    ...
    private Task WaitForLanguageMenuTransitionAsync() => _languageMenuTransition;

    private Task WaitForDeleteWarningTransitionAsync() => _deleteWarningTransition;
}
